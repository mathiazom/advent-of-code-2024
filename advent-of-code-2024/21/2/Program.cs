...
namespace advent_of_code_2024._21._2;
...
public class Program : ISolution
{
    public void Run(string path)
    {
        ...
        var sum = codes.Aggregate(new BigInteger(0),
            (acc, c) => acc + CodeCost(c, numPathsLookup, dirPathsLookup, memo));

        Console.WriteLine(sum);
        Console.WriteLine(sum == 218309335714068);
    }

    private static BigInteger CodeCost(string code,
    ...
    {
        var nums = code.Select(c => c switch
        {
            '0' => PadState._0,
            '1' => PadState._1,
            '2' => PadState._2,
        ...
        {
            cost += numPathsLookup[num][prev].Select(candidate => RecurseCodeOption(1, candidate, dirPathsLookup, memo))
                .Min();
            prev = num;
        }

        return cost * int.Parse(code[..^1]);
    }

    private static BigInteger RecurseCodeOption(int count, List<PadState> path,
        Dictionary<PadState, Dictionary<PadState, List<List<PadState>>>> dirPathsLookup,
        Dictionary<string, BigInteger> memo)
    {
        var memoKey = count + ":" + string.Join(",", path);
        if (memo.TryGetValue(memoKey, out var o)) return o;

        BigInteger cost = 0;
        var s = PadState.A;
        foreach (var p in path)
        {
            var paths = dirPathsLookup[p][s];
            if (count == 25)
            {
                cost += paths[0].Count;
                s = p;
            ...
            cost += paths
                .Select(path2 => RecurseCodeOption(count + 1, path2, dirPathsLookup, memo)).Min();
            s = p;
        }
